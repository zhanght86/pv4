...
try{
	...
	if( document.all.devlist != null ) {
		if( document.all.devlist.length != undefined ) {
			...
			var devList=document.getElementsByName("devlist");
			for( i=0; i<devList.length; i++ ) {
				if( devList[i].checked == true ) {
					flag = true;
				}
			}
			if( !flag ) {
	...
	flag = false;
	var qseqList=document.getElementsByName("qseq");
	for( i=0; i<qseqList.length; i++ ) {
		if( qseqList[i].checked ) {
			flag = true;
		}
	}
...
flag = false;
	var qbegin =$("#qbegin"); 
	if( (qbegin.val()=="") || !isDate(qbegin.val()) ) {
		alert("请输入正确的<bean:message key='pvxp.jytj.begin'/>\n\n长度不正确");
		return false;
	}else if( !isNumber(qbegin.val()) ) {
		alert( "请输入正确的<bean:message key='pvxp.jytj.begin'/>\n\n不应包含有非数字字符" );
		qbegin.focus();
		return false;
	}else if( (parseInt(qbegin.val()) < parseInt(b)) || (parseInt(qbegin.val()) > parseInt(e)) ) {
		alert( "请输入正确的<bean:message key='pvxp.jytj.begin'/>\n\n超出可统计范围" );
		qbegin.focus();
	...
	var qend = $("#qend"); 
	if( (qend.val()=="") || !isDate(qend.val()) ) {
		alert( "请输入正确的<bean:message key='pvxp.jytj.end'/>\n\n长度不正确" );
		qend.focus();
		return false;
	}else if( !isNumber(qend.val()) ) {
		alert( "请输入正确的<bean:message key='pvxp.jytj.end'/>\n\n不应包含有非数字字符" );
		qend.focus();
		return false;
	}else if( (parseInt(qend.val()) < parseInt(b)) || (parseInt(qend.val()) > parseInt(e)) ) {
		alert( "请输入正确的<bean:message key='pvxp.jytj.end'/>\n\n超出可统计范围" );
		qend.focus();
		return false;
	}
	...
	var qunit = $("#qunit"); 
//处理统计单位
	for( i=0; i<qunit.val().length; i++ ) {
		if( qunit[i].checked == true ) {
			flag = true;
		}
	}
	if( !flag ) {
...
<body onload="javascript:parent.hidit();">
	<div align="center">
		<form name="JymlMDeal_form" action="JymlMDeal.do" target="pvmain"
			...
			<table width="100%" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td>
						...
						<font class="location">设备交易情况明细表</font>
					</td>
 ...
 %> </a>
			...
			<table width="100%" cellspacing="1" cellpadding="3"
				...
				<tr>
					...
					<td class="list_td_detail" width="30%">
						&nbsp;
						<input type="text"  id="qbegin" name="qbegin" size="16" maxlength="8">
					</td>
					<td class="list_td_prom" width="20%" align="center">
					...
					<td class="list_td_detail" width="30%">
						&nbsp;
						<input type="text" id="qend" name="qend"  size="16" maxlength="8">
					</td>
				</tr>
				<tr class="list_tr1">
			...
			<table width="100%" cellspacing="1" cellpadding="3"
				...
				<tr>
					...
					<td class="list_td_title" width="30%">
						&nbsp;
						<input type="checkbox" name="qunit" id="qunit" value="1">
						成功&nbsp;&nbsp;
						<input type="checkbox" name="qunit" id="qunit" value="2">
						失败
					</td>
				</tr>
