...
//验证日期是否正确
	...
	for( i=0; i<document.all.outboxstatus.length; i++ ) {
			if( document.all.outboxstatus(i).checked ) {
				flag = true;
			}
		}
		if( !flag ) {
			alert( "请选择钞箱状态！" );
			document.all.outboxstatus(0).focus();
			return false;
		}else {
			flag = false;
		}
	for( i=0; i<document.all.tradestatus.length; i++ ) {
			if( document.all.tradestatus(i).checked ) {
				flag = true;
			}
		}
		if( !flag ) {
			alert( "请选择交易状态！" );
			document.all.tradestatus(0).focus();
			return false;
		}else {
			flag = false;
		}
...
<body onload="javascript:parent.hidit();">
	<div align="center">
		<html:form  action="/CashDetailAdd.do" target="pvmain"
			...
			<table width="100%" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td>
						...
						<font class="location">现金交易明细统计报表</font>
					</td>
 ...
 %> </a>
			...
			<table width="100%" cellspacing="1" cellpadding="3"
				...
				<tr>
					...
					<td class="list_td_detail" width="30%">
						<input type="text" name="batchid" id="batchid" size="16"
							maxlength="25" >
					</td>
				</tr>
				<!-- 日期 -->
