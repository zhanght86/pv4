...
var hassall = false;
function selectAll(){
	if( document.all.typelist != null ){
		if( document.all.typelist.length != undefined ){
			if( hassall ){
				for( i=0;i<document.all.typelist.length;i++ ){
					if( document.all.typelist[i].checked == true ) document.all.typelist[i].checked=false;
				}
				hassall = false;
				document.all.sallbt1.src="images/default/bt_selectall.gif";
			}else{
				for( i=0;i<document.all.typelist.length;i++ ){
					if( document.all.typelist[i].checked == false ) document.all.typelist[i].checked=true;
				}
				hassall = true;
			...
			if( hassall ){
				if( document.all.typelist.checked == true ) document.all.typelist.checked=false;
				hassall = false;
				document.all.sallbt1.src="images/default/bt_selectall.gif";
			}else{
				if( document.all.typelist.checked == false ) document.all.typelist.checked=true;
				hassall = true;
				document.all.sallbt1.src="images/default/bt_remove.gif";
			}
		}
...
<body onload="javascript:parent.hidit();">
	<div align="center">
		<form name="JytjTypeMDeal_form" action="JytjTypeMDeal.do"
			...
			<table width="100%" cellspacing="1" cellpadding="2"
				...
				<%
							...
							int typelen = myDTypeList.size();
							int subtrnum = typelen / 5;
							if (typelen % 5 > 0)
						subtrnum++;
				...
				<tr class="list_tr<%=i % 2%>">
					<%
								...
								int thisnum = i * 5 + j;
								if (thisnum < typelen) {
									Devtype tmpdtype = (Devtype) myDTypeList.get(thisnum);
									String tpname = charSet.db2web(myPubUtil.dealNull(tmpdtype.getDevname())).trim();
									String tpno = myPubUtil.dealNull(tmpdtype.getTypeno()).trim();
					%>
					<td width="20%">
						&nbsp;
						<input name="typelist" id="typelist" type="checkbox"
							value="<%=tpno%>">
						&nbsp;
						<%=tpname%>
					</td>
			...
			<table width="100%" cellspacing="1" cellpadding="3"
				...
				<tr align="center">
					<td class="list_td_title" colspan="4">
						<b><bean:message key="pvxp.jytj.input" />
						</b>
					</td>
				</tr>
				<tr>
			...
			<table width="100%" cellspacing="1" cellpadding="3"
				...
				<tr>
					<td class="list_td_title" width="20%" align="center">
						<b><bean:message key="pvxp.jytj.statmode" />
						</b>
					</td>
					...
					<td class="list_td_title" width="20%" align="center">
						<b><bean:message key="pvxp.jytj.statpoint" />
						</b>
					</td>
					<td class="list_td_title" width="30%">
						&nbsp;
