...
/*处理统计单位
	for( i=0; i<document.all.moneytype.length; i++ ) {
		if( document.all.moneytype(i).checked == true ) {
			flag = true;
		}
	}
	if( !flag ) {
		alert( "请选择一种货币类型" );
		document.all.moneytype(0).focus();
		return false;
	}else {
		flag = false;
	}
...
详细出处参考：http://www.jb51.net/article/16848.htm
...
<div align="center">
<form name="JytjDevMDeal_form" action="JytjDevMDeal.do" target="pvmain" method="post" onsubmit="javascript:return dealFormSubmit();">
...
<table width="100%" cellspacing="0" cellpadding="0">
 <tr>
  ...
  <td>
    <font color=blue>PowerView XP </font>--- <font class="location"><bean:message key="pvxp.jytj.input"/></font>
  </td>
...
<table width="100%" cellspacing="1" cellpadding="3" class="list_table_border">
  <tr align="center">
    <td class="list_td_title" colspan="4"><b><bean:message key="pvxp.jytj.input"/></b></td>
  </tr>
  <tr>
...
<table width="100%" cellspacing="1" cellpadding="3" class="list_table_border">
  <tr>
    <td class="list_td_title" width="20%" align="center"><b><bean:message key="pvxp.jytj.statmode"/></b></td>
    <td class="list_td_title" width="30%">&nbsp;
      <select name="statmode" id="statmode" onChange="javascript:dealSortMode(this);" onMouseWheel="javascript:return false;">
        <option value="all"><bean:message key="pvxp.jytj.statall"/></option>
<%
	try{
...
%>
        <option value="<%=i%>"><%=myPubUtil.ReadConfig("TradeType","Type"+i+"_Name","",braf)%></option>
<%
	}
%>
    </td>
    
    <td class="list_td_title" width="20%" align="center"><b>货币类型</b></td>
	<td class="list_td_title" width="30%">&nbsp;
...
%>
<!--交易分类明细-->
  <tr class="list_tr<%=j%2%>" id="<%="Type"+i+"_"+j%>" style="display:none;">
		<script>
    ...
    <td width="20%" align="center">
    	<b><%=sortname%></b><br>
     	<a href="javascript:chkallfunc<%=i%><%=j%>();" onClick="//parent.showit();" onFocus="this.blur()">
        <img src="images/default/bt_selectall.gif" border="0" name="chkall<%=i%><%=j%>" id="chkall<%=i%><%=j%>" align="absmiddle">
     	</a>
    </td>
...
%>
				<td width="20%">
					<nobr><input type="checkbox" name="trcdlist<%=i%><%=j%>" id="trcdlist<%=i%><%=j%>" value="<%=trcdno%>"><%=trcdnm%></nobr>
				</td>
<%
					}else {
%>
