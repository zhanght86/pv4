...
<body onload="javascript:parent.hidit();">
	<div align="center">
		<form name="JymlMDeal_form" action="cczzjymx.do" target="pvmain"
 ...
 %> </a>
			...
			<table width="100%" cellspacing="1" cellpadding="3"
				...
				<tr>
					...
					<td class="list_td_title" width="30%">
						...
						<select name="statmode" id="statmode"
							...
							<%
										try {
										String[][] tradecode = myPubUtil.Ini2Array("ini", "Trade.ini", "Rate", "Rate_Num", "Rate_", ",", "");
										
										int sortmodenum = Integer.parseInt(myPubUtil.ReadConfig("RateType", "TypeSortNum", "0", braf));
										
										for (int i = 1; i <= sortmodenum; i++) {
							%>
							<option value="<%=i%>">
								<%=myPubUtil.ReadConfig("RateType", "Type" + i + "_Name", "", braf)%>
							</option>
							<%
							}
							%>
					...
					<td class="list_td_title" width="30%">
						&nbsp;
						<input type="radio" name="qunit" id="qunit" value="1" checked
							style="display:none">
						&nbsp;&nbsp;
						<input type="radio" name="qunit" id="qunit" value="2"
							style="display:none">
					</td>
				</tr>
				...
				<%
						for (int i = 1; i <= sortmodenum; i++) {
						int sortnamenum = Integer.parseInt(myPubUtil.ReadConfig("RateType", "Type" + i + "_Num", "0", braf));
						for (int j = 1; j <= sortnamenum; j++) {
							String tmpstr = myPubUtil.ReadConfig("RateType", "Type" + i + "_" + j, "", braf);
