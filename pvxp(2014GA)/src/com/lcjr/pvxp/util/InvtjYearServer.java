 ...
 * @时间 2012-03-26
 */
public class InvtjYearServer extends Thread {

	Logger log = Logger.getLogger("web.lcjr.pvxp.util.InvtjYearServer.java");
				...
				if (devnoList.size() == 0) {
					...
					for (int i = 0; i < invtypeArray.size(); i++) {
						...
						for (int j = 0; j < total; j++) {
							...
							invty = myCharSet.web2db(temp.substring(temp.indexOf(',') + 1));
							if (code.equals(type)) {
								type = invty;
							}
						}
