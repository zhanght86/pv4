 ...
 * 
 */
public class JytjDayServer extends Thread {

	Logger log = Logger.getLogger(JytjDayServer.class.getName());
		...
		try {
			...
			try {
				...
				for (int i = 0; i < pernum; i++) {
					...
					result.addAll((List) myTjDayBean.getQueryList(HQLstr));
				}

				// 将查询结果中所有的币种都保存到 bizhArray 类集中
				for (int i = 0; i < result.size(); i++) {

					myTjDay = (Trcdtjday) result.get(i);

					// 取出币种
					bizh = myTjDay.getMoneytype().trim();
					if (bizh == null || bizh.equals("")) {
						bizh = "001";
					}
				...
				for (int i = 0; i < devnoList.size(); i++) {
					...
					for (int j = 0; j < result.size(); j++) {
						...
						myTjDay = (Trcdtjday) result.get(j);
						if (devno.equals(myTjDay.getDevno().trim())) {
							// 将交易次数和成功交易次数 转换成 长整型
							strnum += Long.parseLong(myTjDay.getStrnum().trim());
							ttrnum += Long.parseLong(myTjDay.getTtrnum().trim());
						}
					}
					// 将交易次数写入到 类集
				...
				for (int i = 0; i < devnoList.size(); i++) {
					...
					for (int j = 0; j < bizhArray.size(); j++) {
						...
						for (int k = 0; k < result.size(); k++) {
							...
							myTjDay = (Trcdtjday) result.get(k);
							tmpbizh = myTjDay.getMoneytype().trim();

							if (devno.equals(myTjDay.getDevno().trim())) {
								if (bizh.equals(tmpbizh) || (bizh.equals("001") && (tmpbizh == null || tmpbizh.equals("")))) {

									tmppr1 = myTjDay.getStrsum().trim();
					...
					// 将每台设备交易金额数组按先后顺序排列到数组中，显示的时候按顺序输出
				 ...
				 * = (List)DevTtrsumVector.get(i); for( int j=0;
				 * j<tempStrsumList.size(); j++ ) { ResultString +=
				 * tempStrsumList.get(j) + "|" + tempTtrsumList.get(j) + "," ; }
				 * 
