 ...
 * 
 */
public class JytjDayAServer extends Thread {

	Logger log = Logger.getLogger(JytjDayAServer.class.getName());
	...
	public void run() {
		...
		try {
			...
			try {
				...
				AutostaModel.updateAutosta(myAs);
				log.info("自 交易统计 日 结束");
			} catch (HibernateException he) {
				log.error(" HibernateException 报错1", he);
				try {
					StaMissionModel myStaMissionModel = new StaMissionModel();
					StaMissionBean myStaMissionBean = new StaMissionBean();
					StaMission myStaMission = new StaMission();
