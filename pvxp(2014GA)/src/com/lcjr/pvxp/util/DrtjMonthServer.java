...
public class DrtjMonthServer extends Thread {
			...
			try {
				...
				for (int i = 0; i < devs; i++) {
					...
					for (int k = 0; k < resize; k++) {
						...
						if (devno.equals(myDevMonth.getDevno().trim()) && statno.equals("Z001")) {
							timelen += Double.parseDouble(myDevMonth.getTimelen().trim());
						}
					}
				...
				String tmpqend = "";
				String tmpqbegin = qbegin + "01";
				String[] mon = { "31", "28", "31", "30", "31", "30", "31", "31", "30", "31", "30", "31" };
				int tmpyear = Integer.parseInt(qend.substring(0, 4));
				int tmpmonth = Integer.parseInt(qend.substring(4, 6));
				if (tmpyear % 4 == 0 && tmpmonth == 2) {
					tmpqend = qend + "29";
				} else {
					tmpqend = qend + mon[tmpmonth - 1];
				}
				
				for (int i = 0; i < 365 * 4; i++) {
					tmpstr = myPubUtil.getOtherDate(tmpqend, -i);
					if (tmpstr.equals(tmpqbegin)) {
						dtdiff = i + 1;
						break;
					}
				}
