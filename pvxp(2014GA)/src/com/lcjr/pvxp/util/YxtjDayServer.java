 ...
 * 
 */
public class YxtjDayServer extends Thread {
	
	Logger log = Logger.getLogger("web.lcjr.pvxp.util.YxtjDayServer.java");
	...
	public void run() {
		...
		try {
			...
			try {
				...
				/*
				 * //取出设备编号 for( int i=0; i<result.size(); i++ ) { myTjDay =
				 * (Trcdtjday)result.get(i); devno = myTjDay.getDevno().trim();
				 * iflag = 0; for( int j=0; j<devnoArray.size(); j++ ) { if(
					...
					for (int j = 0; j < sortlist.length; j++) {
						...
						for (int k = 0; k < tmplist.size(); k++) {
							...
							for (int l = 0; l < result.size(); l++) {
								myTjDay = (Trcdtjday) result.get(l);
								if (devno.equals(myTjDay.getDevno().trim()) && trcdtype.equals(myTjDay.getTrcdtype().trim())) {
				...
				for (int i = 0; i < sortlist.length; i++) {
					...
					for (int j = 0; j < tmplist.size(); j++) {
						...
						for (int k = 0; k < result.size(); k++) {
							myTjDay = (Trcdtjday) result.get(k);
							if (trcdtype.equals(myTjDay.getTrcdtype().trim())) {
								strnum += Long.parseLong(myTjDay.getStrnum().trim());
								ttrnum += Long.parseLong(myTjDay.getTtrnum().trim());
							}
						}
				...
				if (devnoList.size() == 0) {
					...
					outstr.append("\t\t<td width='10%' class='list_td_prom'><b>安装地址</b></td>\n");
					for (int i = 0; i < trcdnmlist.size(); i++) {
						outstr.append("\t\t<td width='" + (100 - 15) / (trcdnmlist.size() + 1) + "%' class='list_td_prom'><b>" + (String) trcdnmlist.get(i) + "</b></td>\n");
					}
				...
				// 记录文件
				String filename = "M_02_" + operid + "_" + DateTimeStr + ".htm";
				...
				// 更新系统任务表
				myStaMission = (StaMission) myStaMissionModel.getOneStaMission(DateTimeStr, "02", "M", operid);
