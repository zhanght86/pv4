 ...
 * 
 */
public class JytjBankMonthServer extends Thread {

	Logger log = Logger.getLogger(JytjBankMonthServer.class.getName());
		...
		try {
			...
			try {
				...
				for (int z = 0; z < HQLstr.size(); z++) {
					...
					result = (List) myTjMonthBean.getQueryList(tmpHQLstr);
											...
											if (trcdno.equals(myTjMonth.getTrcdtype().trim())) {
												...
												for (int y = 0; y < bizhArray.size(); y++) {
													...
													if (bizh.equals(moneyno)) {
														try {
															// wukp1129 ftmp1 =
															// Double.parseDouble(myTjMonth.getStrsum().trim());
															// wukp1129 ftmp2 =
														...
														// if(ftmp1!=0){
														// System.out.println("tablePriceContent["+j+"]["+x+"]的值是："+tablePriceContent[j][x]+"
														// totalPrice["+x+"]的值是："+totalPrice[x]);
														// }
													}
												}
											}
