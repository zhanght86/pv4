...
public class MqtjDayServer extends Thread {

	Logger log = Logger.getLogger("web.lcjr.pvxp.util.MqtjDayServer.java");
	 ...
	 * 
	 */
	private List mqtjState = new ArrayList();
	...
	/**
	 * @return the mqtjState
	 */
	public List getMqtjState() {
		return mqtjState;
	}

	/**
	 * @param mqtjState
	 *            the mqtjState to set
	 */
	public void setMqtjState(List mqtjState) {
		this.mqtjState = mqtjState;
	}

	public MqtjDayServer() {
			...
			try {
				...
				for (int i = 0; i < devnoList.size(); i++) {
					...
					for (int j = 0; j < subdevice.size(); j++) {
						...
						for (int t = 0; t < mqtjState.size(); t++) {
							tmpstate = (String) mqtjState.get(t);
							times = 0;
				...
				if (devnoList.size() == 0) {
					...
					// 部件
					for (int i = 0; i < subdevice.size(); i++) {
						outstr.append("\t\t<td class='list_td_prom' colspan='" + mqtjState.size() + "'><b>");
					...
					outstr.append("\t<tr align='center'>\n");
					for (int i = 0; i < subdevice.size(); i++) {
						for (int j = 0; j < mqtjState.size(); j++) {
							tmpstate = ((String) mqtjState.get(j)).trim();
					...
					for (int i = 0; i < devnoList.size(); i++) {
						...
						for (int j = 0; j < subdevice.size(); j++) {
							for (int k = 0; k < mqtjState.size(); k++) {
								outstr.append("\t\t<td class='list_td_title' align='center'><nobr>&nbsp;" + devtimes.get(i * subdevice.size() * mqtjState.size() + j * mqtjState.size() + k)
										+ "</nobr></td>\n");
							}
						}
						outstr.append("\t</tr>\n");
					...
					for (int i = 0; i < subdevice.size(); i++) {
						for (int j = 0; j < mqtjState.size(); j++) {
							outstr.append("\t\t<td class='list_td_prom' align='center'>&nbsp;<b>" + parttimes.get(i * mqtjState.size() + j) + "</b></td>\n");
						}
					}
					outstr.append("\t</tr>\n");
