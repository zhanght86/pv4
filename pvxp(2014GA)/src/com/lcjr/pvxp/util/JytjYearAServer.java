 ...
 * 
 */
public class JytjYearAServer extends Thread {
	
	Logger log = Logger.getLogger("web.lcjr.pvxp.util.JytjYearAServer.java");
	...
	public JytjYearAServer() {
		
	}
	
	
	public void run() {
		// 进入主线程 自动统计报表 交易统计 （年报）
		...
		try {
			...
			try {
				...
				try {
					...
					myStaMission = (StaMission) myStaMissionModel.getOneStaMission(DateTimeStr, "01", "A", operid);
					myStaMission.setCurrentflag("2");
					myStaMission.setStatename(myStaMission.getStatename().trim());
					myStaMissionBean.updateStaMission(myStaMission);
				} catch (Exception e) {
