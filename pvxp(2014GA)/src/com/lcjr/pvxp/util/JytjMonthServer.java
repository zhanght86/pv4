 ...
 * 
 */
public class JytjMonthServer extends Thread {
	
	Logger log = Logger.getLogger("web.lcjr.pvxp.util.JytjMonthServer.java");
		...
		try {
			...
			try {
				...
				for (int i = 0; i < result.size(); i++) {
					...
					/*
					 * //取出设备编号 devno = myTjDay.getDevno().trim(); iflag = 0;
					 * for( int j=0; j<devnoArray.size(); j++ ) { if(
					 * devno.equals( devnoArray.get(j) ) ) { iflag = 1; break; } }
					 * if( iflag == 0 ) { devnoArray.add( devno ); }
					 */
				...
				// 计算每台设备的交易量
				/*
				 * for( int i=0; i<devnoArray.size(); i++ ) { devno =
				 * (String)devnoArray.get(i); strnum = 0; ttrnum = 0; for( int
				 * j=0; j<result.size(); j++ ) { myTjDay =
				 * (Trcdtjday)result.get(j); if( devno.equals(
				 * myTjDay.getDevno().trim() ) ) { strnum += Long.parseLong(
				 * myTjDay.getStrnum().trim() ); ttrnum += Long.parseLong(
				 * myTjDay.getTtrnum().trim() ); } } strnumArray.add(
				 * Long.toString( strnum ) ); ttrnumArray.add( Long.toString(
				 * ttrnum ) ); }
				 */
				for (int i = 0; i < devnoList.size(); i++) {
					devno = (String) devnoList.get(i);
				...
				/*
				 * for( int i=0; i<devnoArray.size(); i++ ) { devno =
				 * (String)devnoArray.get(i); strsumArray = new ArrayList();
				 * ttrsumArray = new ArrayList(); for( int j=0; j<bizhArray.size();
				 * j++ ) { bizh = (String)bizhArray.get(j); strsum = 0.00;
				 * ttrsum = 0.00; for( int k=0; k<result.size(); k++ ) {
				 * myTjDay = (Trcdtjday)result.get(k); if( devno.equals(
				 * myTjDay.getDevno().trim() ) && bizh.equals(
				 * myTjDay.getMoneytype().trim() ) ) { strsum +=
				 * Double.parseDouble( myTjDay.getStrsum().trim() ); ttrsum +=
				 * Double.parseDouble( myTjDay.getTtrsum().trim() ); } }
				 * strsumArray.add( myPubUtil.double2String( strsum, 2 ) );
				 * ttrsumArray.add( myPubUtil.double2String( ttrsum, 2 ) ); }
				 * DevStrsumVector.add( strsumArray ); DevTtrsumVector.add(
				 * ttrsumArray ); }
				 */
				for (int i = 0; i < devnoList.size(); i++) {
					devno = (String) devnoList.get(i);
				...
				// 处理统计后的数据
				/*
				 * List tempStrsumList = new ArrayList(); List tempTtrsumList =
				 * new ArrayList(); for( int i=0; i<devnoArray.size(); i++ ) {
				 * ResultString = ""; ResultString += devnoArray.get(i) + ",";
				 * ResultString += ttrnumArray.get(i) + ","; ResultString +=
				 * strnumArray.get(i) + ",";
				 * 
				 * tempStrsumList = (List)DevStrsumVector.get(i); tempTtrsumList =
				 * (List)DevTtrsumVector.get(i); for( int j=0; j<tempStrsumList.size();
				 * j++ ) { ResultString += tempStrsumList.get(j) + "|" +
				 * tempTtrsumList.get(j) + "," ; }
				 * 
