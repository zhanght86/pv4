 ...
 * @time 2012-03-26
 */
public class JytjDevDayServer extends Thread {

	Logger log = Logger.getLogger(JytjDevDayServer.class.getName());
	...
	public void run() {
		...
		try {
			...
			String moneyno = "";
			String tmpHQLstr = "";
			// wukp28 double ftmp1 = 0.00;
			// wukp28 double ftmp2 = 0.00;
			long ftmp1 = 0;
			long ftmp2 = 0;
			int strnum = 0;
			...
			try {
				...
				// wukp20111202 long num1=0;
				// 根据查询结果 进行统计
				for (int i = 0; i < result.size(); i++) {
					myTjDay = (Trcdtjday) result.get(i);
					// wukp20111202 用于向数据库中添加数据 for(int k=0;k<10;k++){
					// num1=myPubUtil.str2long(myTjDay.getTtrnum().trim())+1;
					// myTjDay.setTtrnum(myPubUtil.long2str10000(num1));
					// myTjDayBean.addTrcdtjday(myTjDay);
					//
					// wukp20111202 }
				...
				if (devnoList.size() == 0) {
					...
					for (int i = 1; i <= sortnum; i++) {
						int typenum1 = Integer.parseInt(myPubUtil.ReadConfig("TradeType", "Type1_Num", "0", "Trade.ini"));

						for (int j = 1; j <= typenum1; j++) {
							String tmpstr1 = myPubUtil.ReadConfig("TradeType", "Type1_" + j, "0", "Trade.ini");
					...
					outstr.append("\t\t<td  colspan='" + (totalsortnum + 1) * 2 + "'  class='list_td_prom' ><b>点击次数</b></td>\n");
					outstr.append("\t</tr>\n");
					...
					for (int i = 0; i < devnoList.size(); i++) {
						...
						for (int j = 0; j < totalsortnum; j++) {
							// wukp1128
							tmpString = myPubUtil.long2str10000(sortpriceContent[i][j]); // 缩小10000倍
						...
						// wukp1128
						tmpString = myPubUtil.long2str10000(devTotalprice[i]);
						outstr.append("\t\t<td class='list_td_title'>&nbsp;<b>" + tmpString + "</b></td>\n");
						outstr.append("\t</tr>\n");
					}
					...
					outstr.append("\t\t<td  rowspan='2'  class='list_td_prom' ><b>所属机构</b></td>\n");
					outstr.append("\t\t<td  rowspan='2'  class='list_td_prom' ><b>安装地址</b></td>\n");
					outstr.append("\t\t<td  colspan='" + (tradecode.length / 2 + 1) * 2 + "'  class='list_td_prom' ><b>设备交易</b></td>\n");
					outstr.append("\t</tr>\n");
