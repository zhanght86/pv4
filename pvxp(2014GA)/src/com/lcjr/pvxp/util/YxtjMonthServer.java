 ...
 * 
 */
public class YxtjMonthServer extends Thread {
	
	Logger log = Logger.getLogger("web.lcjr.pvxp.util.YxtjMonthServer.java");
	...
	public void run() {
		...
		try {
			...
			try {
				...
				/*
				 * //取出设备编号 for( int i=0; i<result.size(); i++ ) { myTjMonth =
				 * (Trcdtjmonth)result.get(i); devno =
				 * myTjMonth.getDevno().trim(); iflag = 0; for( int j=0; j<devnoArray.size();
				 * j++ ) { if( devno.equals( devnoArray.get(j) ) ) { iflag = 1;
				 * break; } } if( iflag == 0 ) { devnoArray.add( devno ); } }
				 */
					...
					for (int j = 0; j < sortlist.length; j++) {
						...
						for (int k = 0; k < tmplist.size(); k++) {
							...
							for (int l = 0; l < result.size(); l++) {
								myTjMonth = (Trcdtjmonth) result.get(l);
								if (devno.equals(myTjMonth.getDevno().trim()) && trcdtype.equals(myTjMonth.getTrcdtype().trim())) {
				...
				for (int i = 0; i < sortlist.length; i++) {
					...
					for (int j = 0; j < tmplist.size(); j++) {
						...
						for (int k = 0; k < result.size(); k++) {
							myTjMonth = (Trcdtjmonth) result.get(k);
							if (trcdtype.equals(myTjMonth.getTrcdtype().trim())) {
								strnum += Long.parseLong(myTjMonth.getStrnum().trim());
								ttrnum += Long.parseLong(myTjMonth.getTtrnum().trim());
							}
						}
