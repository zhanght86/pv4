 ...
 * 
 */
public class DevtjMonthAServer extends Thread {
	
	Logger log = Logger.getLogger("web.lcjr.pvxp.util.DevtjMonthAServer.java");
...
//		自动统计报表 设备故障统计 （月报）
		...
		try {
			...
			try {
				...
				// 执行查询
				DevtjmonthBean myDevMonthBean = new DevtjmonthBean();
				Devtjmonth myDevMonth = new Devtjmonth();
				
				
				// result = (List)myDevMonthBean.getQueryList( HQLstr );
				// wukp 20120301
				// 处理大量设备编号代码
				// 分批次进行查询
				// 将结果添加到类集reault中
				...
				for (int i = 0; i < pernum; i++) {
					...
					result.addAll((List) myDevMonthBean.getQueryList(HQLstr));
				}
				
				
				// 取出设备编号和设备类型
				for (int i = 0; i < result.size(); i++) {
					myDevMonth = (Devtjmonth) result.get(i);
					
					
					// 取出设备类型
					statno = myDevMonth.getStateno().trim();
					if (!statno.equals("")) {
						iflag = 0;
				...
				for (int i = 0; i < devnoList.size(); i++) {
					...
					for (int j = 0; j < statArray.size(); j++) {
						...
						for (int k = 0; k < result.size(); k++) {
							myDevMonth = (Devtjmonth) result.get(k);
							if (devno.equals(myDevMonth.getDevno().trim()) && statno.equals(myDevMonth.getStateno().trim())) {
								
								
								// wukp20120302 计算设备通讯故障时间
								if (statno.equals("Z001")) {
									Z001len += Long.parseLong(myDevMonth.getTimelen().trim());
								} else {
									times += Long.parseLong(myDevMonth.getTimes().trim());
									timelen += Long.parseLong(myDevMonth.getTimelen().trim());
				...
				for (int i = 0; i < statArray.size(); i++) {
					...
					for (int j = 0; j < result.size(); j++) {
						myDevMonth = (Devtjmonth) result.get(j);
						if (statno.equals(myDevMonth.getStateno().trim())) {
							times += Long.parseLong(myDevMonth.getTimes().trim());
							timelen += Long.parseLong(myDevMonth.getTimelen().trim());
						}
					}
				...
				for (int i = 0; i < 365 * 3; i++) {
					tmpstr = myPubUtil.getOtherDate(qend + "30", -i);
					if (tmpstr.equals(qbegin + "01")) {
						dtdiff = i + 1;
						break;
					}
				}
				...
				if ((devnoList.size() == 0) || (statArray.size() == 0)) {
					...
					long bal2 = 0;
					if (statArray.size() == 0) {
						outstr.append("\t<tr class='list_tr1'>\n\t\t<td colspan='4' class='list_td_title' align='center'>&nbsp;<nobr><font color='red'>数据库中没有相关数据..</font></nobr></td>\n \t</tr>\n");
					} else {
				...
				try {
					...
					log.error("HibernateException12", e);
				}
