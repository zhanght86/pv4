 ...
 * 
 */
public class DevtjDayAServer extends Thread {
	...
	Logger log = Logger.getLogger("web.lcjr.pvxp.util.DevtjDayAServer.java");
		...
		try {
			...
			try {
				...
				for (int i = 0; i < result.size(); i++) {
					myDevDay = (Devtjday) result.get(i);
				...
				for (int i = 0; i < devnoList.size(); i++) {
					...
					for (int j = 0; j < statArray.size(); j++) {
						...
						for (int k = 0; k < result.size(); k++) {
							...
							if (devno.equals(myDevDay.getDevno().trim()) && statno.equals(myDevDay.getStateno().trim())) {

								// wukp20120302 计算设备通讯故障时间
								if (statno.equals("Z001")) {
									Z001len += Long.parseLong(myDevDay.getTimelen().trim());
								} else {
									times += Long.parseLong(myDevDay.getTimes().trim());
									timelen += Long.parseLong(myDevDay.getTimelen().trim());
				...
				for (int i = 0; i < statArray.size(); i++) {
					...
					for (int j = 0; j < result.size(); j++) {
						myDevDay = (Devtjday) result.get(j);
						if (statno.equals(myDevDay.getStateno().trim())) {
							times += Long.parseLong(myDevDay.getTimes().trim());
							timelen += Long.parseLong(myDevDay.getTimelen().trim());
						}
					}
				...
				if (!qbegin.equals(qend)) {
					outstr.append("\t\t\t\t" + qbegin.substring(0, 4) + "年" + Integer.parseInt(qbegin.substring(4, 6)) + "月" + Integer.parseInt

					(qbegin.substring(6, 8)) + "日");
					outstr.append("---" + qend.substring(0, 4) + "年" + Integer.parseInt(qend.substring(4, 6)) + "月" + Integer.parseInt(qend.substring
