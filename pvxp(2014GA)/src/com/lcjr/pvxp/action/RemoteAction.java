 ...
 */
public final class RemoteAction extends Action {
	
	Logger log = Logger.getLogger("web.com.lcjr.pvxp.action.RemoteAction.java");
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String devlist[] = (String[]) ((RemoteForm) form).getDevlist();
		String devip = (String) ((RemoteForm) form).getDevip().trim();
			...
			try {
				...
				for (int i = 0; i < devlist.length; i++) {
					...
					if (recvpack == null) {
						...
						result[i] = recvpack;
						myDevinfo.setDevip(devip);
						myDevinfo.setDevaddr(myPubUtil.dealNull(myDevinfo.getDevaddr()).trim());
						
						int flag = DevinfoModel.updateDev(myDevinfo, devlist[i]);
