 ...
 */
@Service
public class SendTradeMessage implements ISendMessage {

	Logger log = Logger.getLogger(SendTradeMessage.class.getName());
	...
	public void sendTradeMsg(String msg, boolean last) {
		...
		for (int i = 0; i < ClientsManager.getClients().size(); i++) {
			...
			if (sess.isOpen() && sess.getUserProperties().get("type").equals("trade"))
				try {
